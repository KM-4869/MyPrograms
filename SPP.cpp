...
bool StandarPointPositioning(OBSERVATION_DATA*Obs_Data, vector<SATPOSRES>* SatPosRes, STAPOSRES* StaPosRes,ACCURACY* PosAccuracy)
{
	...
	for (int IterationTime = 0; IterationTime < 10; IterationTime++)
	{
		...
		for (int i = 0; i < Obs_Data->SatObs.size();)
		{
			...
			//对流层延迟计算
			Trop.push_back(Hopefield(Temperature0, AirPressure0, SeaLevelHeight, RelativeHumidity0, StationPosBlh.H, E));
		...
		//精度评定
		PosAccuracy->sigma0 = Sigma0(Obs_Data->SatObs.size(), XYZT.size(), B, P, l, x);

		CalculateQxx(Obs_Data->SatObs.size(), XYZT.size(), B, P, PosAccuracy->Qxx);
		PosAccuracy->PDOP = sqrt(PosAccuracy->Qxx[0] + PosAccuracy->Qxx[XYZT.size() + 1] + PosAccuracy->Qxx[2 * XYZT.size() + 2]);
...
bool StandarPointVelocity(OBSERVATION_DATA* Obs_Data, vector<SATPOSRES>* SatPosRes, STAPOSRES* StaPosRes, ACCURACY* VelAccuracy)
{
	...
	//精度评定
	CalculateQxx(Obs_Data->SatObs.size(), 4, B, P, VelAccuracy->Qxx);
	VelAccuracy->sigma0 = Sigma0(Obs_Data->SatObs.size(), 4, B, P, l, x);
	//输出测速结果
...
void StaPosRes_Show(GPSTIME* GpsTime, STAPOSRES* StaPosRes, ACCURACY* PosAccuracy, ACCURACY* VelAccuracy, int SatNum, bool PositioningSuccess)
{
	printf("SPP: %d %10.3f  X:%16.6f  Y:%16.6f  Z:%16.6f  B:%12.8f  L:%12.8f  H:%7.3f Vx:%8.5f Vy:%8.5f Vz:%8.5f GPS Clk:%8.4f BDS Clk:%8.4f PosSigma0=%6.3f PDOP:%6.3f  VelSigma0=%7.4f SatNum:%02d", GpsTime->Week, GpsTime->SecOfWeek, StaPosRes->StationPos.Xyz.X, StaPosRes->StationPos.Xyz.Y, StaPosRes->StationPos.Xyz.Z, StaPosRes->StationPosBlh.B, StaPosRes->StationPosBlh.L, StaPosRes->StationPosBlh.H, StaPosRes->StationVel[0], StaPosRes->StationVel[1], StaPosRes->StationVel[2], StaPosRes->ct_R[0], StaPosRes->ct_R[1], PosAccuracy->sigma0, PosAccuracy->PDOP, VelAccuracy->sigma0, SatNum);
