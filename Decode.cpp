...
void RangeDecode(unsigned char* buff, OBSERVATION_DATA* Obs_Data)
{
	...
	for (int i = 0; i < Obs_Data->Obs_Number; i++)
	{
		...
		BinaryToAnytype(&Obs_Data->SatObs[j].CNo, buff + HeadLen + i * 44 + 36);
		BinaryToAnytype(&Obs_Data->SatObs[j].psr[k], buff + HeadLen + i * 44 + 8);
		BinaryToAnytype(&Obs_Data->SatObs[j].adr[k], buff + HeadLen + i * 44 + 20);
...
void PositionDecode(unsigned char* buff, DECODEPOS* Pos)
{
	...
	BinaryToAnytype(&Pos->lat, buff + HeadLen + 8);
	BinaryToAnytype(&Pos->lon, buff + HeadLen + 16);
	BinaryToAnytype(&Pos->hgt, buff + HeadLen + 24);
...
unsigned short NonAteloem7DataDecode(unsigned char* buff, int* RemLen, OBSERVATION_DATA* Obs_Data, vector<EPHEMERIS>* GPSEphem, vector<EPHEMERIS>* BDSEphem, DECODEPOS* Pos)
{
	...
	case 43:RangeDecode(buff, Obs_Data);
		break;
		//GPS星历解码
	case 7:if (GEphContinuous == false)GPSEphem->clear();
		GPSEphem->push_back({});
		GPSEphemDecode(buff, &GPSEphem->back());
		if (GPSEphem->back().health == 1 || abs(GPSEphem->back().toe - Obs_Data->GpsTime) > 7500.0)//解出的星历若不健康或者星历过期，则删去
			GPSEphem->pop_back();
		GEphContinuous = true;
		break;
		//BDS星历解码
	case 1696:if (BEphContinuous == false)BDSEphem->clear();
		BDSEphem->push_back({});
		BDSEphemDecode(buff, &BDSEphem->back());
		if (BDSEphem->back().health == 1 || abs(BDST2GPST(BDSEphem->back().toe) - Obs_Data->GpsTime) > 3900.0)//解出的星历若不健康或者星历过期，则删去
			BDSEphem->pop_back();
		BEphContinuous = true;
		break;
		//位置解码
